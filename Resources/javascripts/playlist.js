...
SC.Playlist.prototype = {
  ...
  generateTracksUrl : function(baseUrl) { // generates the API url based on properties of the playlist
    ...
    if(!baseUrl) { // if no baseUrl then use json
      format = "json";
      baseUrl = "http://localhost:4422/cloud/";
    }
  ...
  save : function() {
    ...
    $.post("http://localhost:4422/cloud/playlists/" + this.id ,{"_method":"PUT","playlist[tracks]":tracks,"playlist[version]":this.version},function(dataJS) {
      var data = eval('(' + dataJS + ')');
  ...
  saveName : function() {
    ...
    $.post("http://localhost:4422/cloud/playlists/" + this.id ,{"_method":"PUT","playlist[name]":this.name},function(dataJS) {
      var data = eval('(' + dataJS + ')');
      if(data.response == 200) {
        self.version++;
  ...
  savePosition : function() {
    ...
    $.post("http://localhost:4422/cloud/playlists/" + this.id ,{"_method":"PUT","playlist[position]":pos},function(dataJS) {
      var data = eval('(' + dataJS + ')');
      if(data.response == 200) {
  ...
  destroy : function() {
    if(this.persisted) {
      $.post("http://localhost:4422/cloud/playlists/" + this.id,{"_method":"DELETE"},function() {
    ...
    $("<li listId='" + this.id + "' class='" + (this.properties.is_owner ? "" : "shared") + " " + (this.properties.playlist.collaborative ? "collaborative" : "") + " " + (this.persisted ? "" : "dont-persist") + " " + (this.properties.playlist.smart ? "smart" : "") + " " + (this.properties.playlist.search ? "search" : "") + "'><span></span><a href='#" + this.name.replace(/\s/, "+") + "'>" + this.name + (this.properties.is_owner ? "" : " <em>by " + this.properties.playlist.owner.nickname + "</em>") + "</a><a class='collaborative' title='Make Playlist Collaborative' href='http://localhost:4422/cloud/playlists/" + this.id + "'>&nbsp;</a><a class='share' title='Share Playlist' href='/share/" + this.properties.playlist.share_hash + "'>&nbsp;</a><a class='delete' title='Remove Playlist' href='http://localhost:4422/cloud/playlists/" + this.id + "'>&nbsp;</a></li>")
      ...
      .find('a.collaborative').click(function() {
        if(!$(this).parents("li").hasClass("shared")) {
          $.post("http://localhost:4422/cloud/playlists/" + self.id ,{"_method":"PUT","playlist[collaborative]":!self.properties.playlist.collaborative,"version":self.version},function() {
