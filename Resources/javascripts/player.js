...
SC.Player.prototype = {
  ...
  initialize: function() {
    ...
    $("#pl-create-form").submit(function(ev) {
      ...
      $.post("http://localhost:4422/cloud/playlists",props,function(data) {
        var pl = eval('(' + data + ')');
...
//          self.removePlaylist("search1");
    ...
    $("#add-playlist").click(function(ev) {
      if($("body").hasClass("logged-in")) {
        ...
        $.post("http://localhost:4422/cloud/playlists",{'playlist[name]':"Untitled playlist",'playlist[position]': pos},function(data) {
          var item = eval('(' + data + ')');
    ...
    if($("body").hasClass("logged-in")) {
      ...
      $.getJSON("http://localhost:4422/cloud/playlists",function(playlists) {
